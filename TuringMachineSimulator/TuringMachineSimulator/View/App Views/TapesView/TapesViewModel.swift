...
class TapesViewModel: ObservableObject {
    
    func addTape(algorithm: Algorithm, viewContext: NSManagedObjectContext) {
        if algorithm.wrappedTapes.isEmpty {
            let tape = Tape(context: viewContext)
            tape.id = 0
            algorithm.addToTapes(tape)
            addComponents(tape: tape, viewContext: viewContext)
            algorithm.addToTapes(tape)
            ...
            do {
                ...
                print("New tape saved successfully.")
            } catch {
                print("Failed saving new tape.")
        ...
        // Getting the name
        let nameIDArray = algorithm.wrappedTapes.map { $0.wrappedID }
